...
- `supporters[]` - Supporter (donors/external) 🔸 **BREADCRUMB**
- `events[]` - Events
...
### `OrgMember` ⭐ **PRIMARY CONTACT MODEL**
...
### `Supporter` 🔸 **BREADCRUMB FOR FUTURE**
...
### `EventTask` 🎯 **ASANA-STYLE TASK MANAGEMENT**
...
**Generated by:**
- `SetupEvent.jsx` survey → `EventTaskSuggestions.jsx`
- User picks tasks to add
- Future: OpenAI generates custom tasks

---

### `EventLocation` 🏢 **REUSABLE VENUES**
...
```
1. Import CSV → Create Supporters
2. Tag Supporters → "Bros&Brews_Invited"
3. Create Event
...
### Dashboard → Create Event → Invite People
...
**Active Models:**
- ✅ `OrgMember` - Master contact list (everyone)
- ✅ `EventAttendee` - Event participation tracking
- ✅ `Event` - Events/fundraisers
- ✅ `EventTask` - Asana-style tasks
- ✅ `EventLocation` - Reusable venues

**Breadcrumb Models (Future):**
- 🔸 `Supporter` - External donors (nonprofits)
- 🔸 `EventPipelineEntry` - Legacy pipeline (may deprecate)
